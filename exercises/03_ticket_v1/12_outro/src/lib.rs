...
impl Order {
    fn verify_product_name(product_name: &String) {
        if product_name.len() <= 0 || product_name.len()  >= 300 {
            panic!("Product name cannot be empty or larger than 300 bytes");
        }
    }

    fn verify_quantity(quantity: &u32) {
        if *quantity <= 0 {
            panic!("Quantity cannot be equal to or lower than 0");
        }
    ...
    fn verify_unit_price(unit_price: &u32) {
        if *unit_price <= 0 {
            panic!("Unit price cannot be equal to or lower than 0");
        }
    ...
    pub fn set_quantity(&mut self, quantity: u32) {
        Self::verify_quantity(&self.quantity);
